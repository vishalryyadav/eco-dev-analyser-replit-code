...
async function post(endpoint: string, body: unknown) {
  const response = await fetch(endpoint, { method: 'POST', headers: { 'content-type': 'application/json', 'x-ecodev-client': 'vscode' }, body: JSON.stringify(body) });
...
async function analyze(code: string, language: string, execute: boolean, profile: string) {
  return post(endpoint(), { code, language, execute, preferences: { profile, instruction: profilePrompt(profile) } });
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const run = async (selectionOnly: boolean) => {
    ...
    try {
      await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'EcoDev analyzing code…' }, async () => {
        ...
        if ((data.analysis?.alternatives ?? []).length) {
          const pick = await vscode.window.showQuickPick(data.analysis.alternatives.slice(0, 6).map((alt: any) => ({ label: alt.title, description: `${alt.complexity} · ${alt.expectedRuntimeChange} · ${alt.expectedMemoryChange}`, detail: `${alt.projectedEnergyChange} · ${alt.projectedCarbonChange}`, alt })), { placeHolder: 'Choose an optimization approach to inspect its trade-offs', ignoreFocusOut: true });
          if (pick) { output.appendLine(`\nSelected alternative: ${pick.alt.title}`); output.appendLine(`Complexity: ${pick.alt.complexity}`); output.appendLine(`Runtime: ${pick.alt.expectedRuntimeChange}`); output.appendLine(`Memory: ${pick.alt.expectedMemoryChange}`); output.appendLine(`Energy: ${pick.alt.projectedEnergyChange}`); output.appendLine(`Carbon: ${pick.alt.projectedCarbonChange}`); output.show(true); }
        }
      });
    } catch (error) {
  ...
  const scanWorkspace = async () => {
    ...
    if (!workspace) return vscode.window.showInformationMessage('Open a workspace before running a project scan.');
    try {
      await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'EcoDev scanning workspace…' }, async () => {
        const uris = await vscode.workspace.findFiles(SUPPORTED_GLOBS, '**/{node_modules,.git,dist,build,coverage}/**', 200);
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('ecodev.analyzeWorkspace', scanWorkspace),
  ...
  const onSave = vscode.workspace.onDidSaveTextDocument(async (document) => {
    ...
    try { const data = await analyze(document.getText(), language, Boolean(vscode.workspace.getConfiguration('ecodev').get('executeCode')), String(vscode.workspace.getConfiguration('ecodev').get('profile') || 'balanced')); output.appendLine(`On-save ${document.fileName}: ${data.analysis?.score ?? 'n/a'}/100 · ${data.analysis?.complexity?.time ?? 'complexity unknown'}`); }
    catch (error) { output.appendLine(`On-save error: ${error instanceof Error ? error.message : 'unknown error'}`); }
  });
