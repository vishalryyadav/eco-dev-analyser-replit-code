...
router.post("/report", (req, res) => {
  ...
  const lines = [
    ...
    "## Runtime", `- Status: ${r.execution?.status ?? r.compile?.status ?? "not run"}`, `- Wall time: ${r.execution?.wallTimeMs ?? "n/a"} ms`, `- CPU time: ${r.execution?.cpuTimeMs ?? "n/a"} ms`, `- Peak memory: ${r.execution?.peakMemoryKb ?? "n/a"} KB`, `- Energy: ${r.eco?.energyWh ?? "n/a"} Wh (estimate)`, `- Carbon: ${r.eco?.carbonGrams ?? "n/a"} gCO2e (estimate)`, "",
