...
router.post("/analyze", async (req, res) => {
  try {
    ...
    if (runEnabled) {
      const firstRun = await executeCode(body.code, analysis.language);
      compile = firstRun.compile;
      execution = firstRun.execution;
      if (iterationsRaw > 1 && execution?.status === "completed") {
        const runs = [execution];
        for (let i = 1; i < iterationsRaw; i++) {
          const next = await executeCode(body.code, analysis.language);
          if (next.execution?.status !== "completed") break;
          runs.push(next.execution);
        }
        const valid = runs.filter((r): r is ExecutionResult => r.wallTimeMs != null);
        if (valid.length > 1) {
          const sorted = valid.map(r => r.wallTimeMs as number).sort((a, b) => a - b);
          const median = sorted[Math.floor(sorted.length / 2)];
          const cpu = valid.map(r => r.cpuTimeMs).filter((v): v is number => v != null);
          const rss = valid.map(r => r.peakMemoryKb).filter((v): v is number => v != null);
    ...
    if (typeof body.compareCode === "string" && body.compareCode.trim()) {
      if (Buffer.byteLength(body.compareCode, "utf8") > MAX_CODE_BYTES) return res.status(413).json({ error: `compareCode exceeds the ${MAX_CODE_BYTES} byte limit` });
      compare = body.compareCode;
    }
    if (compare) {
      ...
      compareAnalysis.alternatives = rankAlternatives(compareAnalysis.alternatives, profile);
