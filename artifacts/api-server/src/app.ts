...
function rateLimit(req: Request, res: Response, next: NextFunction) {
  ...
  if (bucket.count > maxRequests) return res.status(429).json({ error: "Too many requests. Please retry after the rate-limit window." });
  if (buckets.size > 10_000) for (const [k, v] of buckets) if (now - v.started >= windowMs) buckets.delete(k);
  return next();
}
