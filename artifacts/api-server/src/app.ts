...
if (existsSync(frontendDist)) { app.use(express.static(frontendDist, { index: "index.html", maxAge: "1h" })); app.use((req, res, next) => { if (req.method === "GET" && !req.path.startsWith("/api/") && req.accepts("html")) return res.sendFile(path.join(frontendDist, "index.html")); return next(); }); }
