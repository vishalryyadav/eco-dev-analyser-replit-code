...
export type ExecutionResult = { status: ExecutionStatus; stdout: string; stderr: string; exitCode: number | null; wallTimeMs: number | null; cpuTimeMs: number | null; peakMemoryKb: number | null; measured: boolean; sandbox: string };
...
function commandLine(args: string[]) { return args.map(quote).join(" "); }
...
function buildCommand(runtime: "bubblewrap" | "firejail", args: string[], cwd: string) {
  ...
  if (runtime === "bubblewrap") {
    const binds: string[] = ["--ro-bind", "/usr", "/usr", "--ro-bind", "/bin", "/bin", "--ro-bind", "/lib", "/lib", "--proc", "/proc", "--dev", "/dev", "--tmpfs", "/tmp", "--bind", cwd, "/workspace", "--chdir", "/workspace"];
    ...
    return { command: "bwrap", args: ["--die-with-parent", "--unshare-all", "--new-session", ...binds, "--clearenv", "--setenv", "PATH", "/usr/local/bin:/usr/bin:/bin", "--setenv", "HOME", "/tmp", "--setenv", "LANG", "C.UTF-8", "--setenv", "LC_ALL", "C.UTF-8", "--", "sh", "-c", limited] };
  }
  return { command: "firejail", args: ["--quiet", "--private", "--net=none", "--caps.drop=all", "--noroot", `--rlimit-as=${MEMORY_MB * 1024}`, "--", "sh", "-c", limited] };
}
...
function parseRss(stderr: string) { const m = stderr.match(/Maximum resident set size \(kbytes\):\s*(\d+)/i); return m ? Number(m[1]) : null; }
...
async function runOne(runtime: "bubblewrap" | "firejail", args: string[], cwd: string, timeoutMs: number): Promise<ExecutionResult> {
  ...
  return new Promise((resolve) => {
    const child = spawn(wrapped.command, wrapped.args, { cwd: runtime === "bubblewrap" ? "/" : cwd, env: { PATH: "/usr/local/bin:/usr/bin:/bin", HOME: "/tmp", LANG: "C.UTF-8", LC_ALL: "C.UTF-8" }, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "", stderr = "", timedOut = false, outputKilled = false;
    child.stdout.on("data", chunk => { stdout += chunk.toString(); if (stdout.length > 32_000 && !outputKilled) { outputKilled = true; child.kill("SIGKILL"); } });
    child.stderr.on("data", chunk => { stderr += chunk.toString(); if (stderr.length > 32_000 && !outputKilled) { outputKilled = true; child.kill("SIGKILL"); } });
    const timer = setTimeout(() => { timedOut = true; child.kill("SIGKILL"); }, timeoutMs);
    child.on("error", error => { clearTimeout(timer); resolve({ status: "runtime_error", stdout: stdout.slice(0, 32_000), stderr: `${stderr}${error.message}`.slice(0, 32_000), exitCode: null, wallTimeMs: Number(process.hrtime.bigint() - started) / 1e6, cpuTimeMs: null, peakMemoryKb: null, measured: false, sandbox: runtime }); });
    child.on("close", code => {
      clearTimeout(timer);
      const wall = Number(process.hrtime.bigint() - started) / 1e6;
      resolve({ status: timedOut ? "timeout" : code === 0 ? "completed" : "runtime_error", stdout: stdout.slice(0, 32_000), stderr: stderr.slice(0, 32_000), exitCode: code, wallTimeMs: wall, cpuTimeMs: parseCpu(stderr) ?? (code === 0 && !timedOut ? wall : null), peakMemoryKb: parseRss(stderr), measured: true, sandbox: runtime });
    });
  });
}
...
export async function executeCode(code: string, language: SupportedLanguage): Promise<SandboxRun> {
  ...
  if (!sandbox) {
    const unavailable: ExecutionResult = { status: "sandbox_unavailable", stdout: "", stderr: "Secure execution is disabled because neither Bubblewrap nor Firejail is available. Static analysis remains available.", exitCode: null, wallTimeMs: null, cpuTimeMs: null, peakMemoryKb: null, measured: false, sandbox: "none" };
